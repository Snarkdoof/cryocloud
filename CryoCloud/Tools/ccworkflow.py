...
class Pebble:
    ...
    def __init__(self, gid=None):
        self.gid = gid
        if gid is None:
            self.gid = struct.unpack("I", os.urandom(3))[0]
        self.resolved = []
        self.args = {}
        self.retval_dict = {}
        self.nodename = {}
...
class Task:
    def __init__(self, workflow, taskid=None):
        ...
        self.taskid = taskid
        if taskid is None:
            self.taskid = struct.unpack("I", os.urandom(3))[0]
        self._upstreams = []
        self._downstreams = []
        self.priority = 50
        ...
        self.runOn = "always"
        self.runIf = None
        self.name = "task%d" % struct.unpack("I", os.urandom(3))[0]
        self.module = None
        self.config = None
                ...
                if "type" in self.args[arg]:
                    if self.args[arg]["type"] == "file":
                        ...
                        if "id" in self.args[arg]:
                            ...
                            _tempid = struct.unpack("I", os.urandom(3))[0]
...
class WorkflowHandler(CryoCloud.DefaultHandler):
    ...
    def _addJob(self, n, lvl, taskid, args, module, jobtype,
                itemid, workdir, priority, node, log_prefix=None):
        ...
        if node.splitOn:
            ...
            for x in range(len(origargs)):
                ...
                if x < len(origargs) - 1:
                    ...
                    subpebble.gid = struct.unpack("I", os.urandom(3))[0]
                    ...
                    taskid = struct.unpack("I", os.urandom(3))[0]
                    ...
                    i = self._addJob(node, lvl, taskid, args, module=mod, jobtype=jobt,
                                     ...
                                     priority=runtime_info["priority"],
                                     node=runtime_info["node"], log_prefix=pebble.gid)
                    subpebble.jobid = i
