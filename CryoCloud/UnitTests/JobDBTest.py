...
from CryoCloud.Common.jobdb import *

stop_event = threading.Event()


class FakeWorker(threading.Thread):
    def __init__(self, func, workertype, max_jobs):
        super().__init__()
        self.func = func
...
class JobDBTest(unittest.TestCase):
    ...
    def testWorker(self):
        ...
        worker = FakeWorker(process, TYPE_NORMAL, 10)
        worker.start()
    ...
    def testWorkerMulti(self):
        ...
        FakeWorker(process, TYPE_NORMAL, 1).start()
        FakeWorker(process, TYPE_NORMAL, 1).start()
        FakeWorker(process, TYPE_NORMAL, 1).start()
        FakeWorker(process, TYPE_ADMIN, 10).start()
        FakeWorker(process, TYPE_ADMIN, 2).start()
    ...
    def testWorkerInstant(self):
        ...
        worker = FakeWorker(process, TYPE_NORMAL, 50)
        worker.start()
